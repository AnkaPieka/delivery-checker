...
export function checkDelivery(deliveries: deliveriesType, path: pathType) {
  ...
  const deliveriesWithPositions: newDeliveryCheckType[][] = deliveries.map(
    (delivery) =>
  ...
  for (const deliveryPair of deliveriesWithPositions) {
  ...
  for (const delivery of deliveriesWithPositions) {
